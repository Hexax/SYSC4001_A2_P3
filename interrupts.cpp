...
std::tuple<std::string, std::string, int> simulate_trace(std::vector<std::string> trace_file, int time, std::vector<std::string> vectors, std::vector<int> delays, std::vector<external_file> external_files, PCB current, std::vector<PCB> wait_queue) {
  ...
  for (size_t i = 0; i < trace_file.size(); i++) {
    ...
    if (activity == "CPU") { // As per Assignment 1
      ...
      int programSize = get_size(program_name, external_files);
      if (programSize < 0)
        programSize = 0;
      ...
      int loader = programSize * 15;
      execution += std::to_string(current_time) + ", " + std::to_string(loader) + ", loading program into memory\n";
      current_time += loader;
      ...
      {
        auto [exec, exec_sys, exec_end] = simulate_trace(exec_traces, current_time, vectors, delays, external_files, current, wait_queue);
